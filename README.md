...
앞 네 플랫폼은 **verdict severity 대수를 공유**한다: Certstra의 `certifiable < needs_review < blocked`는
Attestra `valid/thin/breach`와 정렬(`to_attestra_verdict`) → 인증 결과를 Attestra가 증언 가능.
Scorestra는 verdict가 아니라 등급 tier(가중 score→분류)를 내는 5번째 형제(2번째 Condense emit).
